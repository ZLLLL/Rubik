...
public class LabelMaker {
	...
	private int mStrikeWidth; // 宽度
	private int mStrikeHeight; // 高度
	private boolean mFullColor; // 是否要全部颜色
	private Bitmap mBitmap; // 位图
	private Canvas mCanvas; // 画布
	private Paint mClearPaint; // 画笔
	...
	public void initialize(GL10 gl) {
		...
		//制定生成N个纹理，第一个参数制定生成1个纹理
		//textures数据将负责存储所有纹理的代号
		gl.glGenTextures(1, textures, 0);
		//获取textures数组中的第一个纹理
		mTextureID = textures[0];
		//通知OpenGL将mTextureID纹理绑定到GL10.GL_TEXTURE_2D中
	...
	public int add(GL10 gl, Drawable background, String text, Paint textPaint,
		...
		checkState(STATE_ADDING, STATE_ADDING);	//检测LabelMaker的mState是不是STATE_ADDING
	...
	public void endDrawing(GL10 gl) {
		...
		gl.glDisable(GL10.GL_BLEND);	//停用混合功能
	...
	/**
	 * 检测LabelMaker的mState是不是oldState，如果不是，就抛出异常，如果是就把mState赋值成newState
	 * @param oldState
	 * @param newState
	 */
