...
public class Ray {
	...
	public void transform(Matrix4f matrix, Ray out) {
		...
		v0.set(mvOrigin);	//把mvOrigin的x,y,z赋值给v0
		v1.set(mvOrigin);	//把mvOrigin的x,y,z赋值给v1
		v1.add(mvDirection);	//v1的x,y,z由mvOrigin和mvDirection的x,y,z对应相加得到

		matrix.transform(v0, v0);
		matrix.transform(v1, v1);

		out.mvOrigin.set(v0);	//把v0的x,y,z坐标设置成out的原点坐标
		v1.sub(v0);
		v1.normalize();
	...
	public void draw(GL10 gl) {
		...
		if (AppConfig.gbTrianglePicked) {	//有三角形被选中
			gl.glColor4f(1.0f, 0.0f, 0.0f, 1.0f);
		} else {	//没有三角形被选中
			gl.glColor4f(0.0f, 1.0f, 0.0f, 1.0f);
		}

		gl.glPointSize(4.0f);	//设置点的大小
		gl.glLineWidth(4.0f);	//设置线的粗细
		gl.glDisable(GL10.GL_DEPTH_TEST);	//停用深度测试

		gl.glEnableClientState(GL10.GL_VERTEX_ARRAY);	//启用顶点坐标数据
		gl.glVertexPointer(3, GL10.GL_FLOAT, 0, gBufPosition);	//设置顶点的位置数据
		gl.glDrawArrays(GL10.GL_POINTS, 0, 2);

		gl.glDisableClientState(GL10.GL_VERTEX_ARRAY);	//停用顶点坐标数据

		gl.glEnable(GL10.GL_DEPTH_TEST);	//启用深度测试
		gl.glPointSize(1f);	//设置点的大小
		gl.glLineWidth(1.0f);	//设置线的粗细
	}
...
/*		//射线原点到圆点中心点的矢量
		...
		diff.sub(mvOrigin, center);	//diff是圆心指向射线原点的向量
		float a = diff.dot(diff) - radius * radius;
		if(a <= 0.0f) {	//射线原点到圆心的距离 < 圆的半径
			return true;
		}
		else {	//射线原点到圆心的距离 > 圆的半径
			float b = mvDirection.dot(diff);
			if(b >= 0.0f) {	//射线原点到圆心的向量与射线方向的夹角 > 90度,因为diff是圆心指向射线原点的向量，所以注意方向
				return false;
			}
			return b * b >= a;
		}
